...
- The config/agent `python3` check exits 0 only when both config surfaces bind each custom role to the expected `./agents/<role>.toml` file, and all five `/Users/maxibon/.codex/agents/*.toml` files have the expected `name`, `sandbox_mode`, and `developer_instructions` fields.
