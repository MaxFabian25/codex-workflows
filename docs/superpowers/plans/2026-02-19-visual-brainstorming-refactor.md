...
```bash
cd "$(git rev-parse --show-toplevel)" && node tests/brainstorm-server/server.test.js
```
...
Search the codebase:
```bash
grep -r "wait-for-feedback" "$(git rev-parse --show-toplevel)"/ --include="*.js" --include="*.md" --include="*.sh" --include="*.json"
```
...
```bash
cd "$(git rev-parse --show-toplevel)" && skills/brainstorming/scripts/start-server.sh --project-dir /tmp/brainstorm-smoke-test
```
...
```bash
grep -r "wait-for-feedback\|sendToClaude\|feedback-footer\|send-to-claude\|TaskOutput.*block.*true" "$(git rev-parse --show-toplevel)"/ --include="*.js" --include="*.md" --include="*.sh" --include="*.html" | grep -v node_modules | grep -v RELEASE-NOTES | grep -v "\.md:.*spec\|plan"
```
