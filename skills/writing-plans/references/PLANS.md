...
When implementing an executable specification (ExecPlan), do not prompt the user for "next steps"; simply proceed to the next milestone. Keep all sections up to date, add or split entries in the list at every stopping point to affirmatively state the progress made and next steps. Resolve ambiguities autonomously. Commit in small, reviewable increments when user instructions and repository policy permit; otherwise record checkpoint progress in the ExecPlan without creating commits.
...
Interpret the key words "MUST", "MUST NOT", "SHOULD", "SHOULD NOT", and "MAY" in this document as described in RFC 2119 and RFC 8174, when and only when they appear in all capitals.
...
At minimum, each milestone should include one verification evidence entry with these fields:

    Command: <exact command run>
    Working directory: <repo-relative or absolute path>
    Expected result: <observable success condition>
    Observed result: <short output excerpt or behavior observed>
    Exit status: <code>
    Timestamp (UTC): <YYYY-MM-DD HH:MMZ>

## Safe-Default Guardrails

Prefer additive, reversible changes by default. Do not run destructive or irreversible operations unless they are explicitly required by the task and permitted by user instructions and repository policy. When requirements are ambiguous, choose the safest viable behavior-preserving path and document the assumption in the `Decision Log`. If you are blocked by an irreversible, security-sensitive, privacy-sensitive, or billing-sensitive decision, ask one targeted question and describe what changes based on the answer.
...
## Skeleton of a Good ExecPlan
    ...
    For each milestone, include at least one verification evidence entry using this template:
