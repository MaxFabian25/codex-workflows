---
name: writing-plans
description: Use when you have an approved spec for a multi-step task and need an implementation plan before touching code
---
...
1. Read the approved spec, relevant repo docs, and existing code patterns before writing tasks.
2. If the repo already uses `.agents/PLANS.md`, a top-level `PLANS.md`, or the user explicitly asks for an ExecPlan, switch to the compatible references in `references/`, follow that planning standard exactly, and validate with `python3 references/validate_execplan.py <plan-file> --strict-evidence` when that workflow requires strict evidence.
